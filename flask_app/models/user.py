...
class User:
    ...
    def create_payment(cls, data):
        query = "INSERT INTO payments (amount, payment_method, status, user_id, event_id) VALUES (%(amount)s, %(payment_method)s, %(status)s, %(user_id)s, %(event_id)s);"
        return connectToMySQL(cls.db_name).query_db(query, data)

    @classmethod
    def get_all_user_payments(cls, user_id):
        query = "SELECT * FROM payments WHERE user_id = %(user_id)s;"
